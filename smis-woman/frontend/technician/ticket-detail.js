...
function renderTicket(ticket) {
  ...
  // Authenticated download helper for technician shell
...
async function renderTimeline() {
  ...
  timeline.innerHTML = items.map((item) => `
    <article class="timeline-item timeline-item-${item.type}">
      ...
      ${item.attachmentUrl ? `<p><a class="response-attachment" href="${escapeHtml(buildAttachmentUrl(item.attachmentUrl))}" target="_blank" rel="noopener noreferrer">View attachment</a></p>` : ''}
    </article>
  `).join('');
}
...
function buildAttachmentUrl(url) {
  ...
  const cleanedBase = apiBase ? apiBase.replace(/\/api\/?$/i, '') : '';
    ...
    if (!cleanedBase && window.LIVE_API) cleanedBase = window.LIVE_API.replace(/\/api\/?$/i, '');
